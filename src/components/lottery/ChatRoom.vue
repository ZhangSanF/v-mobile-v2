<template>
  <div class="chat-room">
          ...
          >
            ...
            <div class="lay-content">
              <div class="msg-header">
                ...
                <span></span>
                <span
                  :class="['lvimg user-lv-' + item.chat_level,userChatId == item.clientId ? 'type-right' : 'type-left']"></span>
                <span></span>
              ...
              <div class="Bubble" :class="'Bubble_'+item.content_type" v-if="item.content_type == 'img' || item.content_type == 'gif'">
                <img
                  class="msg-img"
                  :src="item.content"
                  alt="点击放大图片"
                ...
                <div class="follwOrders">
                  <div class="betHeaderBox">
                    ...
                    <div class="betBox">
                      <div
                        class="timeDown"
                      >封盘：{{timeStampFunc(curTime[item.orders.code+item.chat_id])}}
                      </div>
                      <div
                        class="betBtn"
    ...
    <div class="emojiArea" v-show="emojiAreaShow">
      <div class="emojiTabs">
        <a class="emojiTab" :class="{selected: emojiType === 'emoji'}" @click="emojiChange('emoji')">emoji</a>
        <a class="emojiTab" :class="{selected: emojiType === 'gif'}" @click="emojiChange('gif')" v-if="gifEmojiArray && gifEmojiArray.length">gif表情</a>
      </div>
        ...
        <swiper :aspect-ratio="320/750" :dots-position="'bottom'">
          <swiper-item class="swiper-demo-img" v-for="(item, index) in gifEmojiArray" :key="index">
            <div v-for="(col, index) in item" class="gifCol">
              <a class="gifItem"
                 v-for="(emoji, index) of col"
                 @click="chooseGif(emoji)"
              >
                <img :src="emoji.dir">
                <p>{{emoji.cn_name}}</p>
              </a>
    ...
    <div class="inputArea">
      <div class="inputBtns">
        <span class="iconfont" @click="emojiAreaShow = !emojiAreaShow">&#xe63b;</span>
      ...
      getRedBag(e) {
        ...
        this.homeChatGetRedPackage({red_package_id: this.resBagId}).then(
          res => {
            ...
            if (res.resCode === 1000 && parseInt(res.data) > 0) {
              this.redBagStep = 2;
              this.redBagMoney = res.data;
            } else if (res.resCode === 1000 && parseInt(res.data) === 0) {
              this.redBagStep = 3;
            } else {
              this.redBagStep = 4;
...
<style scoped lang="scss">
  ...
  .emojiArea {
    ...
    .gif-emoji-container {
      ...
      text-align: center;
      /deep/ .vux-slider > .vux-indicator, .vux-slider .vux-indicator-right {
        left: px2rem(300px);
        bottom: px2rem(-2px);
      }
      .gifCol {

    ...
    <div class="main-content page">
      <header class="public-header-background-color header">
        ...
        <span class="leftBtn icon-con" @click="moreGames">
          ...
          <span class="iconfont icon-xiala2" :class="{onShow: moreGamesShow}">&#xe63e;</span>
        </span>
      ...
      <div class="inner">
          ...
          <flexbox :gutter="0">
            ...
            <flexbox-item :order="99" class="code-content font-light-color">
              ...
              <span class="openResultButton" @click="openResultListVisibility">
                <span class="icon resultIcon iconfont">&#xe61d;</span>
        ...
        <div
          ...
          :class="{betContentMoveDown: openResultListShow}"
        >
          <chat-room v-if="areaTab == 'chat'" @changeTabIndex="changeTabIndex" @chartMsg="chartMsg"
                     :msg_content="chat.msgContent"></chat-room>
        </div>
      ...
      <div class="fastMoney" v-show="betOrders.length">
        ...
        <div class="moneyBtns">
          <span class="moneyBtnItem" v-for="(item, index) of fastMoneyArray" :key="index" @click="setFastMoney(item)">{{item}}</span>
        </div>
        ...
        <div class="orders-content">
          ...
          <div class="orders-table-con">
            <x-table :cell-bordered="false" style="background-color:#fff;" class="orders-table">
              ...
              <tbody>
              <tr v-for="(item, index) of betOrders" :key="index">
                <td>{{item.name}}</td>
        ...
        this.homeChatSendMsgToChatRoom({content: this.chat.msgContent}).then(
          res => {
            if (res.resCode === 1000) {
              ...
              this.$vux.alert.show({
                title: "发送失败",
                content: `消息发送失败，请刷新页面重试，如果重复出现此问题请联系客服处理！<br>[code:${
                  res.resCode
                  }][msg:${res.msg}]`
              });
            }
            ...
            _this.homeChatSendMsgToChatRoom(param).then(res => {
              if (res.resCode !== 1000) {
                _this.$vux.alert.show({
                  title: "发送失败",
                  content: `图片发送失败，请刷新页面重试，如果重复出现此问题请联系客服处理！<br>[code:${
                    res.resCode
                    }][msg:${res.msg}]`
                });
              }

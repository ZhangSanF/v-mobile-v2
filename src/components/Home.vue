<template>
  <div>
    <div class="header" v-if="page != 'ucenter'">
      ...
      <div class="rightBox">
        <span v-if="user.username" class="username" @click="tabChange('ucenter')">{{user.username == 'guest' ? '游客' : user.username}}</span>
    ...
    <div :class="page == 'ucenter' ? 'ucBody bg-ucBody-bgColor' : 'body'">
      <keep-alive>
        <router-view @tabsChange="tabsChange"></router-view>
      </keep-alive>
    ...
    <div class="footer row">
      <tabbar>
        ...
        <tabbar-item :selected="page == 'chat'" @click.native="tabChange('chat')" v-if="user.username && !baseInfo.isAgent">
          <span slot="label" class="tab-item-icon">
            <span class="iconfont">&#xe636;</span>
            <p>聊天室</p>
          </span>

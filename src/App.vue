  ...
  export default {
    ...
    mounted() {
      setTimeout(() => {
        ...
        document.getElementsByTagName('body')[0].setAttribute('class', 'skin_' + this.skin);
        this.homeWithOutCheckLoginInit({actions: 'games,name,announcement,links', version: 2})
          .then(res => {
            this.urlParams();
            this.homeIndexInitData({actions: 'publicInfo,userInfo,all'})
              .then((res) => {
                console.log(this.$route.path)
                if (this.lastPath.indexOf('/lottery') >= 0 || this.lastPath.indexOf('/home') >= 0) {
    ...
    methods: {
      ...mapActions(['homeIndexInit', 'homeIndexInitData', 'homeWithOutCheckLoginInit', 'homeChatBind', 'homeIndexQuestNewData']),
        ...
        Socket.onmessage = function (res) {
          ...
          switch (res.type) {
            case 'connect_msg': //初始化
              ...
              _this.homeChatBind({clientId: res.data})
                .then((res) => {
                  if (res.resCode === 1000) {
                    console.log('绑定成功');
                  } else {
            ...
            case 'delete_msg': //删除消息
              if (res.resCode === 1000) {
                _this.$store.commit('DELETE_CHAT_MSG', {id: 'n' + res.data.chat_id});
              }
              break;
            ...
            case 'open_msg': //开奖
              if (res.resCode === 1000) {
                _this.$store.commit('SAVE_OPEN_INFO', {code: res.data.code, data: res.data});
            ...
            case 'time_msg': //时间更新
              if (res.resCode === 1000) {
                for (var k in res.data) {
                  _this.$store.commit('SAVE_TIME_INFO', {code: k, data: res.data[k]});
                }
              }
              break;

            case 'sys_msg': //个人消息推送
              if(res.data.mark && (res.data.mark == 1 && _this.baseInfo.isAgent) || (res.data.mark == 2 && !_this.baseInfo.isAgent) ){
                return;
              }
              if (res.resCode === 1000) {
                _this.$vux.alert.show({
      ...
      oddsUpdate(code) {
        if (code) {
          ...
          if (typeof data[code] !== 'undefined') {
            if (this.code == code) { //当前彩种 更新
              this.$store.commit('DELETE_GAMEDATA_STORE', {code: code});
              this.homeIndexInit({actions: 'playInfo', code: code, page: this.play})
                .then(res => {
      ...
      requestResult(c, p) {
        if (this.nsInfo.indexOf(c) >= 0) {
          this.homeIndexQuestNewData({code: c, phase: p})
            .then(res => {
              if (res.resCode === 1000) {
    ...
    computed: {
      ...mapState(['code', 'user', 'gameDataStore', 'showCode', 'wsLink', 'nsInfo', 'skin', 'play', 'baseInfo']),
